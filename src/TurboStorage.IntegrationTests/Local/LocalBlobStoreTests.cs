...
public sealed class LocalBlobStoreTests : IAsyncLifetime
{
    ...
    public async Task Write_And_Read_RoundTrips()
    {
        ...
        var expectedFilePath = Path.Combine(_tempDir, "test", "file.txt");
    ...
    public async Task Write_CreatesSubdirectories()
    {
        ...
        await Source.Single(new ReadOnlyMemory<byte>(data))
            .RunWith(_store.Write("sub/dir/file.txt"), _materializer);
    ...
    public async Task DeleteAsync_Removes_File()
    {
        ...
        Assert.True(File.Exists(Path.Combine(_tempDir, "delete", "file.txt")));

        await _store.DeleteAsync(["delete/file.txt"], TestContext.Current.CancellationToken);

        Assert.False(File.Exists(Path.Combine(_tempDir, "delete", "file.txt")));
    }
